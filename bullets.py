...
class EnemyBullet:
	def __init__(self, scene, pos, dir, image):
		self.groups = scene.all_sprites, scene.enemy_bullets_SG
		pg.sprite.Sprite.__init__(self, self.groups)
		self.scene = scene
		...
		spread_val =1 #self.scene.player.weapon.spread
		self.vel = dir.rotate(uniform(-spread_val, spread_val)) * 1000
		self.spawn_time = pg.time.get_ticks()
	...
	def check_collision(self):
		target = pg.sprite.spritecollideany(self, self.scene.player_SG)
		if pg.sprite.spritecollideany(self, self.scene.walls_SG):
			self.scene.camera.cameraShake(self.damage, self.damage)
			EnemyExplosion(self.scene, self.pos, self.explosionWalls, scale=self.scale_explosion, destroy=self.destroy_scene)
			self.kill()
		elif target:
			EnemyExplosion(self.scene, self.pos, self.explosionWalls, scale=self.scale_explosion)
			target.entityData.takeDamage(self.get_damage())
			self.kill()
		if pg.time.get_ticks() - self.spawn_time > self.lifetime:
			self.kill()
...
class EnemyGunBullet(EnemyBullet,pg.sprite.Sprite):
	def __init__(self, scene, pos, dir):
		super().__init__(scene, pos, dir, scene.gunBulletImg)
...
class EnemyExplosion(pg.sprite.Sprite):
	...
	def update(self):
		...
		if self.destroy:
			pg.sprite.spritecollide(self, self.scene.walls_SG, True)
		enemycollision = pg.sprite.spritecollide(self, self.scene.player_SG, False)
